...
- (instancetype)init {
    ...
    if (self) {
        ...
        [popupHolder setFrame:CGRectMake(hSpacing,
                                         vSpacing,
                                         self.mdWidth - 2 * hSpacing,
                                         self.mdHeight - 2 * vSpacing)];

        _header = [[MDCalendarDateHeader alloc] initWithFrame:CGRectMake(0,
                                                                         0,
                                                                         popupHolder.mdWidth,
                                                                         kCalendarHeaderHeight)];

        [_header setHeaderColor:[UIColor redColor]];
        [_header setHeaderBackgroundColor:[UIColor redColor]];
        ...
        MDCalendar *calendar = [[MDCalendar alloc]
                                initWithFrame:CGRectMake(0,
                                                         kCalendarHeaderHeight,
                                                         popupHolder.mdWidth,
                                                         popupHolder.mdHeight - kCalendarHeaderHeight -                                                         kCalendarActionBarHeight)];
        ...
        [self.buttonCancel setTitleColor:self.calendar.titleColors[@(MDCalendarCellStateButton)]
                                forState:UIControlStateNormal];
        [self.buttonOk setTitleColor:self.calendar.titleColors[@(MDCalendarCellStateButton)]
                            forState:UIControlStateNormal];
        [self addTarget:self
...
- (void)show {
    ...
    self.hidden = NO;
    self.isOpen = YES;
    [self animateWithDuration:self.timeInterval completion:^(BOOL finished) {
    }];
}
...
- (void)animateWithDuration:(NSTimeInterval)time completion:(void (^ __nullable)(BOOL finished))completion {
    self.alpha = self.isOpen?0:1;
