...
@property (nonatomic) Boolean dimmed;
@end
...
- (void)layoutSubviews {
    ...
    for (UIView *view in tagSubviews_) {
        ...
        if (index != 0) {
            ...
            tempViewFrame.origin.x = prevView.frame.origin.x + prevView.frame.size.width + 7;
        } else {
...
- (void)reloadTagSubviews {
    ...
    for (NSString *tag in _tags) {
        ...
        tagLabel.backgroundColor = self.dimmed ? [UIColor lightGrayColor] : self.labelBackgroundColor;
        ...
        if (_mode == TLTagsControlModeEdit) {
            ...
            buttonFrame.size.height = tagInputField_.frame.size.height - 6;
...
- (void)deleteTagButton:(UIButton *)sender {
    ...
    NSInteger index = [tagSubviews_ indexOfObject:view];
    [self.deleteDelegate deleteButtonClicked:[_tags objectAtIndex:index]];
